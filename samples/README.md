...
LEASH S3.7 / `JsonCommentHandling.Skip`) but trailing commas in the local
file are not.
...
- **`husky.config.author.json`** - what an app author commits to their
  ...
  to deployment metadata, never identity (LEASH S9.2 anti-redirect).
...
LEASH S5.2 spells it out in detail; the short version is:
